...
export const BlogItem = (props) => {
  ...
  return (
    <div className="mr-6 last:mr-0 w-full lg:w-1/3">
      <div className="flex">
        {tag.map((tag) => (
          <div key={tag} className="rounded-full border border-blue-300 bg-blue-50 text-xs font-semibold text-blue-800 border-solid px-2.5 py-0.5 mb-4 w-fit mr-3">
            {tag}
          </div>
        ))}
      </div>
      <h3 className="text-red-500 text-xl h-14">{title}</h3>
