...
Reactions have `shouldPush` set to `false` that indicates the notification should not be sent. This means that reactions do not trigger push notifications as long as the notification server respects this flag.
