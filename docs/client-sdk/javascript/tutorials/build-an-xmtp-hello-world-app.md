...
* A code editor of your choice to build the app.


## Step 1. Create a vanilla React app[​](http://localhost:3000/docs/client-sdk/javascript/tutorials/get-started-react#1-create-a-react-app)
...
All subsequent steps in this tutorial are about adding code to this vanilla React app to build each feature of the "hello world" app.
...
2. To run the vanilla React app, in the root of your newly created my-app directory, run:
   ...
   ![alt_text](img/vanilla-cra.png "image_tooltip")<!--jha retake this with the browser address bar displaying-->


## Step 2. Build the Connect wallet button functionality
...
3. In the existing **src** directory, create a **contexts** folder. In the **contexts** folder, create a file named `WalletContext.js`. Copy the following code and paste it into the file:
    ...
    const connectWallet = async () => {
     ...
     //saul: not sure if await is needed here?
    ...
    <!--Saul: this sounds scarier than it is-->
    The purpose of `WalletContext.js` is to efficiently provide global access to wallet-based values wherever the values are needed in the XMTP hello world app.

    To learn more about Context in React, see [Context](https://reactjs.org/docs/context.html).

4. In the **src** directory, update the existing **App.js** file to use the `WalletContext` you just created and add the **Connect wallet** button UI element, as shown in the highlighted lines of code. Copy the following copy and paste it into the file, replacing all existing code.<!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/e81d4b36b87c63c94cc33801cfbb0f91970b8940-->
...
5. In the **src** directory, update the existing **index.js** file to import the `WalletContextProvider` you created in a previous step and wrap it around your app so that your app can access the provided wallet data, as shown in the highlighted lines of code. Copy the following code and paste it into the file, replacing all existing code.<!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/e81d4b36b87c63c94cc33801cfbb0f91970b8940#diff-bfe9874d239014961b1ae4e89875a6155667db834a410aaaa2ebe3cf89820556-->
...
4. In the root directory of your React app, run `npm start` to start your app. The vanilla Learn React UI should be replaced by a **Connect wallet** button.
   ...
   ![alt_text](img/connect-wallet-running.png "image_tooltip")

5. Click the **Connect wallet** button to open your wallet app extension and connect a blockchain account address to the app. The UI remains the same, but in your wallet app you can see that your address is now connected to your app running at `localhost:3000`:

  ![alt_text](img/connect-wallet-prompt.png "image_tooltip")


## Step 3. Build the Connect to XMTP button functionality
...
The XMTP hello world app enables users to provide these signatures and connect their app to the XMTP network using Connect to XMTP button.

**To build the Connect to XMTP button functionality:**
...
2. In the existing **src/contexts** directory, create a file named **XmtpContext.js**. Copy the following code and paste it into the file:<!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/35c8cded3d20816cd06248f2d05c62d79a82c2f3-->
    ...
    The purpose of `XmtpContext.js` is to efficiently provide global access to XMTP-based values wherever the values are needed in the XMTP hello world app.<!--take a look at a friendlier way to explain this-->

    To learn more about Context in React, see [Context](https://reactjs.org/docs/context.html).

3. In the **src** directory, update the existing **App.js** file to use the `XmtpContext` you just created and add the **Connect to XMTP** button UI element, as shown in the highlighted lines of code. Copy the following copy and paste it into the file, replacing all existing code. <!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/35c8cded3d20816cd06248f2d05c62d79a82c2f3-->
...
4. In the **src** directory, update the existing **index.js** file to import the `XmtpContextProvider` you created in a previous step and wrap it around your app so that your app can access information provided by the XMTP context, as shown in the highlighted lines of code. Note that `XmtpContextProvider` is nested in `WalletContextProvider` because it needs access to the information provided by the wallet app context. Copy the following code and paste it into the file, replacing all existing code. <!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/35c8cded3d20816cd06248f2d05c62d79a82c2f3#diff-bfe9874d239014961b1ae4e89875a6155667db834a410aaaa2ebe3cf89820556-->
...
5. In the root directory of your React app, run `npm start` to start your app. Click Connect wallet to connect your wallet app. The following UI appears:
   ...
   ![alt_text](img/connect-to-xmtp-running.png "image_tooltip")
...
6. Click **Connect to XMTP** to use your desired wallet app to create an XMTP identity (if you don't already have one) and enable your XMTP identity.
   ...
   ![alt_text](img/connect-to-xmtp-prompt.png "image_tooltip")

7. The XMTP hello world app now shows your blockchain account address connected to the XMTP network:

   ![alt_text](img/connected.png "image_tooltip")


## Step 4. Build the Send gm button and display messages functionality
...
For simplicity, the XMTP hello world app enables you to send a preconfigured "gm" message to your already connected address. This makes it easy to see the immediate effects of sending a message.

1. In the **src** directory, update the existing **App.js** file to add a `sendMessage` constant, which sends the message "gm" to the connected wallet address, as shown in the highlighted lines of code. Then create the **Send gm** button UI element that when clicked uses `sendMessage` to send the "gm" message. Copy the following copy and paste it into the file, replacing all existing code. <!--what is `sendMessage`? I called it a constant - but is there something more accurate? And does the button "use" `sendMessage` - is the wording correct?--> <!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/7e2ea197d9516fa93f75b5e866734cd06bf51cdc-->
...
2. In the **src/contexts** directory, update the existing **XmtpContext.js** file to enable the app to display the list of messages your address sends to itself, as shown in the highlighted lines of code. With XMTP, a group of messages sent between two addresses is called a conversation. Copy the following copy and paste it into the file, replacing all existing code.<!--I think this is already there at this point? I think this diff is just removing the code that disables the app from displaying conversations the connected wallet address has with itself. jha verify this...https://github.com/vehidtr/xmtp-chat-tutorial/commit/454c4065b0f2b505780834b55908e2bdf0b2756d. We can probably just call out 56-75 to show how the app displays messages in the conversation?-->
...
3. In the existing **src** directory, create a **hooks** folder. In the **hooks** folder, create a file named **useStreamMessages.js**. Copy the following code and paste it into the file: <!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/cf9f028b18aa2ac825aeeb0b8877bbb6bbcbbdc5-->
    ...
    The purpose of the `useStreamMessages` hook is to enable your app to listen for and display (stream) new messages as they become available. Without using message streaming, you would need to refresh your app to see if any new messages are available.

4. In the **src** directory, update the existing **App.js** file to use the `useStreamMessages` hook you just created, as shown in the highlighted lines of code. Copy the following copy and paste it into the file, replacing all existing code. <!--diff: https://github.com/vehidtr/xmtp-chat-tutorial/commit/cf9f028b18aa2ac825aeeb0b8877bbb6bbcbbdc5#diff-3d74dddefb6e35fbffe3c76ec0712d5c416352d9449e2fcc8210a9dee57dff67-->
...
Congratulations on building your own XMTP hello world app and sending and receiving your first messages with it!
