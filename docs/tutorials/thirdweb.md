---
sidebar_label: Thirdweb attachments
sidebar_position: 4
---
...
# Send remote attachments with XMTP and Thirdweb SDKs
...
![thirdweb Connect Wallet button](img/wallet.png)
...
![Connect to XMTP button](img/xmtp-sign.png)
...
In this case, you can use the XMTP GM Bot and the XMTP instance to create the conversation. If the conversation already exists, it will retrieve its message history.
...
For large attachments above 1MB, use the `RemoteAttachmentCodec`. The codec will automatically encrypt the attachment and upload it to the Thirdweb network. For uploading bigger files, first encrypt them and then upload them to the IPFS network via the Thirdweb SDK.
...
2. Next, upload the file to the IPFS network via the Thirdweb SDK.
...
3. Finally, use the encrypted file's URL to send it to the XMTP network using XMTP ContentTypeRemoteAttachment.
...
With remote storage, it is a different story because uploading and decrypting the file is resource consuming. You will need to use the `RemoteAttachmentCodec` to decrypt the file and then render it.
