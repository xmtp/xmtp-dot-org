...
A user can send and receive encrypted XMTP messages using an app with an embedded XMTP [**client**](/docs/concepts/architectural-overview#client-layer), authenticating using a wallet signature. XMTP network [**nodes**](/docs/concepts/architectural-overview#network-layer) persist the user's messages.
